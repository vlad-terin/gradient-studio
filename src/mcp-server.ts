 ...
 * 
 * A Model Context Protocol server that exposes gradient generation tools
 * for AI agents.
 * 
 * Usage:
 *   npm run mcp
 *   # or
 *   npx @modelcontextprotocol/server-stdio
 * 
 * Tools exposed:
    ...
    case 'generate_gradient':
      return {
        content: [
          {
            type: 'text',
            text: JSON.stringify(generateGradient(args), null, 2),
          },
        ],
      };

    case 'list_modes':
      const modes = Object.entries(shaderConfigs).map(([key, config]) => ({
        id: key,
        name: config.name,
    ...
    case 'list_palettes':
      return {
        content: [
          {
            type: 'text',
            text: JSON.stringify({ palettes: colorPalettes }, null, 2),
          },
        ],
      };
...
async function main() {
  ...
  console.error('Gradient Studio MCP Server running on stdio');
}
